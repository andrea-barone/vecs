...
router.post('/start', async (req: Request, res: Response) => {
  try {
    const { location_id, evse_id, connector_id, auth_id, auth_method, power_kw, auto_increment } = req.body;

    if (!location_id || !evse_id || !connector_id || !auth_id) {
      return res.status(400).json({
        error: 'Missing required fields: location_id, evse_id, connector_id, auth_id',
      });
    }
...
router.post('/full-cycle', async (req: Request, res: Response) => {
  try {
    const { location_id, evse_id, connector_id, auth_id, duration_minutes, power_kw } = req.body;
